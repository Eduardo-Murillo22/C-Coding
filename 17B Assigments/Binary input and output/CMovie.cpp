...
void CMovie::display(){
    ...
    /*
    ifstream Mfile("Movie.dat", ios::in | ios::binary);
    Mfile.read(reinterpret_cast<char*>(movies->mveInfo),movies->nMovies  * sizeof(movies->mveInfo[movies->nMovies]));
    Mfile.close();
    
    for(int i = 0; i < movies->nMovies; i++)
    {
        cout << movies->mveInfo[i].movName << endl;
        cout << movies->mveInfo[i].dirName << endl;
        cout << movies->mveInfo[i].yrRlsd << endl;
        cout << movies->mveInfo[i].runTime << endl;
    }
    */
...
CMovie::CMovie(){
    ...
    ofstream binFile;
    binFile.open("MovieData.dat",ios::in|ios::out|ios::binary);
    ...
    binFile.write(reinterpret_cast<char *>(movies->mveInfo),sizeof(movies->mveInfo));
