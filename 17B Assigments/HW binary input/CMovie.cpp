...
CMovie::CMovie(){
    ...
    txtFile.open("MovieData.txt",ios::in|ios::out);
    binFile.open("MovieData.bin",ios::in|ios::out|ios::binary);
    ...
    for(int mvees=0;mvees<movies->nMovies;mvees++){//Gotta frickin indent!!!!!
        ...
        binFile.write(reinterpret_cast<char *>(movies->mveInfo),sizeof(movies));
